...
public class cameraFollow : MonoBehaviour {
	...
	void FixedUpdate () {
        ...
        if(target != null && !Global.gamePaused)
        {
            ...
            if(targetPos.x > xMax)
            {
                targetPos = new Vector3(xMax, targetPos.y, targetPos.z);
            }
            ...
            if (targetPos.y > yMax)
            {
                targetPos = new Vector3(targetPos.x, yMax, targetPos.z);
            }
            else if (targetPos.y < -yMax)
            {
                targetPos = new Vector3(targetPos.x, -yMax, targetPos.z);
            }
            cameraPos = new Vector3(Mathf.SmoothStep(transform.position.x, targetPos.x, dampTime), Mathf.SmoothStep(transform.position.y, targetPos.y, dampTime));
        }
